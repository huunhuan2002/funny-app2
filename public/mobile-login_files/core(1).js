...
(7400,42-43): run-time error JS1100: Expected ',': =
(7828,42-43): run-time error JS1100: Expected ',': =
 */
/*!
...
$(document).ready(function () {
    ...
    //console.log(s);
    if (s !== 'index' && s !== 'Home' && s !== '' && s !== 'vi-VN' && s !== 'en-US' && s !== 'zh-CN') {
        $('.downloadAppShrtct').hide();
    }
    else {
        $('.downloadAppShrtct').show();
    }
    ...
    //var imgQuickAcctNav = $(".img-nav");
    //$.each(imgQuickAcctNav,
    //    function (index) {
    //        var imgSrc = imgQuickAcctNav[index].getAttribute("src");
    //        imgQuickAcctNav[index].setAttribute("src", imgSrc.replace("-light", ""));
    //    });

    // Deposit FILTER
    if ($("html").attr("lang") == "vi-VN") {

        $(".mgnt-deposit-nav").append("<div class='mgnt-deposit-dropdown'><select id='depositfilter'><option value='Deposit'>Chuyển Tiền</option><option value='Transfer'>Chuyển Quỹ</option><option value='Withdraw'>Rút Tiền</option><option value='History'>Tra Cứu Lịch Sử</option></select ></div >");

    } else {

        $(".mgnt-deposit-nav").append("<div class='mgnt-deposit-dropdown'><select id='depositfilter'><option value='Deposit'>Deposit</option><option value='Transfer'>Transfer</option><option value='Withdraw'>Withdraw</option><option value='History'>History</option></select ></div >");
    ...
    $(document).on("click", function () {
        $(".balanceChild").is(":visible") && $(".balanceChild").slideToggle("active");
    });


    // cLICK TO PUSH UP AND DOWN
    $('.topHolder .doots').on('click', function () {
        if ($('.doots').hasClass('tapMesss')) {
            $('.doots').removeClass('tapMesss');
            $('.topHolder').stop().animate({ top: '0px' }, 300);
        }
        else {
            $('.doots').addClass('tapMesss');
            $('body').removeClass('mobs');
            $('.topHolder').stop().animate({ top: '-100px' }, 300);
        }
    ...
    $('#selectTab').on('change', function () {
        ...
        $("ul.tabHead li").removeClass("active");
        $("." + optTab).addClass("active");
    });


    var click = 0;
    $('.slideResponsive1').on('click', function () {
        click++;
        setTimeout(function () {
            click = 0;
        }, 1000)
        var urlLink = $(this).attr('data-loc');
        if (click == '2') {
            click = 0;
...
InitScripts = (function ($) {
    function onDocumentReady() {
        ...
        $('.changeLang > a > img').attr('src', lang.find('img').attr('src'));
        $('#activeLangText').text(lang.find('span').text());

        $('.activeLang').click(function () {
            $(".mobile-nav").animate({ scrollTop: 0 }, 50);
    ...
    //    $('.fb-mainmenu').click(function () {
    //        $(this).each(function () {
    //            if ($(this).siblings('ul').length) {
    //                valid = false;
    //                $('.mobile-nav').animate({ scrollTop: 0 }, 1);
    //                $(this).siblings('ul').addClass('fb-submenu-animate');  //// show the main menu
    //                $('.fb-mainmenu').addClass('fb-menu-animate'); //// hide the main menu
    //                $('.fb-mainmenu').removeClass('go-back-fb-menu-animate');
    //                $('.fb-submenu').removeClass('go-back-fb-submenu-animate');
    //                $('.mobile-nav').css('overflow', 'hidden');
    //                return false;
    //            }
    //        });
    //    });
    //    $('.fb-go-back').click(function () {
    //        $('.fb-mainmenu').removeClass('fb-menu-animate');
    //        $('.fb-mainmenu').addClass('go-back-fb-menu-animate');  //// show the main menu


    //        $('.fb-submenu').removeClass('fb-submenu-animate');  //// show the main menu
    //        $('.fb-submenu').addClass('go-back-fb-submenu-animate');
...
* Released under the MIT license*/(function (factory) { if (typeof define === 'function' && define.amd) { define(['jquery'], factory); } else { factory(jQuery); } }(function ($) {
...
window.MobileSelect = (function() {
  ...
  function MobileSelect(config) {
    ...
    this.renderWheels(
      this.wheelsData,
      config.cancelTxt || "Cancel",
      config.selectTxt || "Select",
      config.trigger.replace('#','') + '-ms'
    );
    this.displayJson = [];
    this.cascade = false;
    this.startY;
  ...
  MobileSelect.prototype = {
    ...
    init: function(config) {
      ...
      _this.ensureBtn.addEventListener("click", function() {
        ...
        _this.trigger.value = tempValue.replace(/&amp;/g, "&");
    ...
    renderWheels: function(wheelsData, cancelTxt, selectTxt, id) {
      var _this = this;
      _this.mobileSelect = document.createElement("div");
      _this.mobileSelect.id = id;
      ...
      for (var i = 0; i < wheelsData.length; i++) {
        ...
        if (_this.jsonType) {
          for (var j = 0; j < wheelsData[i].data.length; j++) {
            ...
            tempHTML +=
              '<li data-id="' +
              wheelsData[i].data[j].id +
              '">' +
              wheelsData[i].data[j].value +
              "</li>";
          }
        } else {
          for (var j = 0; j < wheelsData[i].data.length; j++) {
